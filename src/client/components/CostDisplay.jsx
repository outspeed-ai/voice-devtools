...
export default function CostDisplay({ costState }) {
  ...
  // Calculate sum of all breakdown costs to verify
  let breakdownSum = 0;
  if (costState.costBreakdown) {
    breakdownSum += costState.costBreakdown.input?.text || 0;
    breakdownSum += costState.costBreakdown.input?.audio || 0;
    breakdownSum += costState.costBreakdown.input?.cached?.text || 0;
    breakdownSum += costState.costBreakdown.input?.cached?.audio || 0;
    breakdownSum += costState.costBreakdown.output?.text || 0;
    breakdownSum += costState.costBreakdown.output?.audio || 0;
  }
    ...
    >
        ...
        {isDurationBased ? (
          ...
          <>
            ...
            <div className="text-right font-medium">
              {formatDuration(costState.durationInSeconds)}
            </div>

            <div className="text-gray-600">Rate:</div>
            <div className="text-right font-medium">
              ${costState.costPerMinute?.toFixed(2) || "0.00"}/minute
            </div>
        ...
        <div className="mt-3 pt-3 border-t border-gray-200">
          ...
          <div className="mb-3">
            ...
            <div className="grid grid-cols-3 gap-1 text-xs ml-2">
              ...
              <div className="text-right">
                {costState.tokenCounts?.input?.text?.toLocaleString() || "0"}
              </div>
              <div className="text-right font-medium">
              ...
              <div className="text-right">
                {costState.tokenCounts?.input?.audio?.toLocaleString() || "0"}
              </div>
              <div className="text-right font-medium">
                $
              ...
              <div className="text-right">
                {costState.tokenCounts?.input?.cached?.text?.toLocaleString() ||
                  "0"}
              </div>
              <div className="text-right font-medium">
                $
                {costState.costBreakdown.input?.cached?.text?.toFixed(6) ||
                  "0.000000"}
              </div>

              <div className="text-gray-600">Cached Audio:</div>
              <div className="text-right">
                {costState.tokenCounts?.input?.cached?.audio?.toLocaleString() ||
                  "0"}
              </div>
              <div className="text-right font-medium">
                $
                {costState.costBreakdown.input?.cached?.audio?.toFixed(6) ||
                  "0.000000"}
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-1 text-xs ml-2">
              ...
              <div className="text-gray-600">Text:</div>
              <div className="text-right">
                {costState.tokenCounts?.output?.text?.toLocaleString() || "0"}
              </div>
              <div className="text-right font-medium">
                $
              ...
              <div className="text-gray-600">Audio:</div>
              <div className="text-right">
                {costState.tokenCounts?.output?.audio?.toLocaleString() || "0"}
              </div>
              <div className="text-right font-medium">
                $
                {costState.costBreakdown.output?.audio?.toFixed(6) ||
                  "0.000000"}
              </div>
            </div>
          </div>
      ...
      {costState.timestamp && (
        <div className="text-xs text-gray-500 mt-2 text-right">
          Last updated: {costState.timestamp}
        </div>
      )}

      {/* to debug the cost breakdown */}
      {/* {!isDurationBased && (
        <div className="text-xs text-red-500 mt-2">
          Debug - Total: ${costState.totalCost.toFixed(6)}, Sum of breakdown: $
          {breakdownSum.toFixed(6)}
        </div>
      )} */}
    </div>
  );
